...
test('ships cannot be placed out-of-bounds', () => {
    expect(() => {
        gameboard.placePiece(outOfBoundsShip, 12, 10).toThrow("Can't place ship out of bounds!");
    })
});

test('a ship with a length larger than the bounds cannot be placed', () => {
    expect(() => {
        gameboard.placePiece(shipWithLengthLongerThanBounds, 3, 5).toThrow("Your ship cannot be placed here!");
    })
});

test('a ship cannot be placed on top of another ship', () => {
    expect(() => {
        gameboard.placePiece(shipWithIdenticalCoords, 5, 5).toThrow("A ship is already placed here!");
    })
});

test('ships should be hittable', () => {
    gameboard.recieveAttack(5, 5);
    expect(gameboard.attacks[0]).toEqual([5, 5]);
});

test('spots are marked as hit if no ship is present', () => {
    gameboard.recieveAttack(10, 10);
    expect(gameboard.attacks[1]).toEqual([10, 10]);
});